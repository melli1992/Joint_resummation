...
C--   Find the zero of this function using DZEROX.
      ...
      IMPLICIT NONE
      INTEGER IORD
      DOUBLE PRECISION FR2, R0, ASI, MC, MB, MT, MUR, ASMUR, ALPHAS_MSTW
      COMMON / DZEROXcommon / FR2, MUR, ASMUR, MC, MB, MT, R0, IORD

      CALL INITALPHASR0(IORD, FR2, R0, ASI, MC, MB, MT)
      FINDALPHASR0 = ALPHAS_MSTW(MUR) - ASMUR ! solve equal to zero
...
C      IVFNS = 0                 ! fixed flavour-number scheme (FFNS)
      NFF = 5                   ! number of flavours for FFNS
...
C----------------------------------------------------------------------
      ...
      DOUBLE PRECISION FUNCTION ALPHAS_MSTW(MUR)
      IMPLICIT NONE
...
*
       ...
       ALPHAS_MSTW = 4.D0*PI*ASF

       RETURN
       END


      DOUBLE COMPLEX FUNCTION ALPHASC_MSTW(MUR)
      IMPLICIT NONE
      INTEGER NFF,IVFNS,NF
      DOUBLE COMPLEX ASCOM,ASF, MUR, R2,M2,R20,LOGNEW,
     &          R2TC, R2BC, R2CC
      DOUBLE PRECISION PI,AS0,M20,ASC,M2C,ASB,M2B,AST,M2T,
     &     ASI,LOGFR
      PARAMETER ( PI = 3.1415 92653 58979 D0 )
*
* ..Input common blocks 
...
*
          NF  = NFF
          R20 = M20 * R2/M2
          ASF = ASCOM(R2, R20, DCMPLX(AS0,0.D0), NF)
*
       ELSE
*
* ..Variable number of flavours
*
          IF (REAL(M2) .GT. M2T) THEN
             NF = 6
             R2TC = M2T * R2/M2
             ASF = ASCOM(R2,R2TC,DCMPLX(AST,0.D0), NF)
*
          ELSE IF (REAL(M2) .GT. M2B) THEN
             NF = 5
             R2BC = M2B * R2/M2
             ASF = ASCOM(R2,R2BC,DCMPLX(ASB,0.D0), NF)
*     
          ELSE IF (REAL(M2) .GT. M2C) THEN
             NF = 4
             R2CC = M2C * R2/M2
             ASF = ASCOM(R2,R2CC,DCMPLX(ASC,0.D0), NF)
*     
          ELSE
             NF = 3
             R20 = M20 * R2/M2
...
*
       END IF
       ALPHASC_MSTW = ASF
c       IF(isnan(REAL(ASF))) THEN
c       ALPHASC_MSTW = DCMPLX(AS0,0.D0)
c       ENDIF 

       RETURN
       END

      DOUBLE COMPLEX FUNCTION ALPHASC_LO(MUR)
      IMPLICIT NONE
      INTEGER NFF,IVFNS,NF
      INTEGER NFMIN, NFMAX, NAORD, NASTPS, K1
      DOUBLE COMPLEX ASCOM,ASF, MUR, R2,M2,R20,LOGNEW,
     &          R2TC, R2BC, R2CC, AS01, LRRAT
      DOUBLE PRECISION PI,AS0,M20,ASC,M2C,ASB,M2B,AST,M2T,
     &     ASI,LOGFR,BETA0, BETA1, BETA2, BETA3
...
*
          NF  = NFF
          R20 = M20 * R2/M2
	  AS01 = DCMPLX(AS0,0.D0)
     	  LRRAT = LOG (R2/R20)
          ASF = AS01 / (1.+ BETA0(NF) * AS01 * LRRAT)
*
       ELSE
*
* ..Variable number of flavours
...
*
      ...
      DOUBLE COMPLEX FUNCTION ASCOM (R2, R20, AS0, NF)
*
      IMPLICIT NONE
      INTEGER NFMIN, NFMAX, NF, NAORD, NASTPS, K1
      DOUBLE PRECISION SXTH, BETA0, BETA1, BETA2, BETA3,
     &      B10, h
      DOUBLE COMPLEX R20, R2,A, AS0,LRRAT,DLR,XK0,XK1,XK2,XK3,CLQ2,
     &    FBETA1,FBETA2,FBETA3
      PARAMETER (NFMIN = 3, NFMAX = 6)
...
*
       IF (NAORD .EQ. 0) THEN
*
         ASCOM = AS0 / (1.+ BETA0(NF) * AS0 * LRRAT)
*
	
       ELSE IF (NAORD .EQ. 1) THEN
*
       DO 2 K1 = 1, NASTPS
         XK0 = DLR * FBETA1 (ASCOM)
         XK1 = DLR * FBETA1 (ASCOM + 0.5 * XK0)
         XK2 = DLR * FBETA1 (ASCOM + 0.5 * XK1)
         XK3 = DLR * FBETA1 (ASCOM + XK2)
         ASCOM = ASCOM + SXTH *(XK0 + 2.* XK1 + 2.* XK2 + XK3)
  2    CONTINUE
*
       ELSE IF (NAORD .EQ. 2) THEN
*
       DO 3 K1 = 1, NASTPS
         XK0 = DLR * FBETA2 (ASCOM)
         XK1 = DLR * FBETA2 (ASCOM + 0.5 * XK0)
         XK2 = DLR * FBETA2 (ASCOM + 0.5 * XK1)
         XK3 = DLR * FBETA2 (ASCOM + XK2)
         ASCOM = ASCOM + SXTH * (XK0 + 2.* XK1 + 2.* XK2 + XK3)
  3    CONTINUE
*  
...
*
       DO 4 K1 = 1, NASTPS
         XK0 = DLR * FBETA3 (ASCOM)
         XK1 = DLR * FBETA3 (ASCOM + 0.5 * XK0)
         XK2 = DLR * FBETA3 (ASCOM + 0.5 * XK1)
         XK3 = DLR * FBETA3 (ASCOM + XK2)
         ASCOM = ASCOM + SXTH * (XK0 + 2.* XK1 + 2.* XK2 + XK3)
  4    CONTINUE
       END IF
*
