...
C                                                                               
       SUBROUTINE PHFNLO(XN, QQ2, UN, DN, SN, CN, GLN)

       IMPLICIT DOUBLE PRECISION (A - Z) 
       DOUBLE COMPLEX UN, DN, SN, CN, GLN, XN, CMU

       DIMENSION LAMBDA(3:6), Q2THR(3)                          
       INTEGER N
CPM       COMMON / SCALESP / Q2S, Q20, Q2THR, LAMBDA                            
cpm       COMMON / SCALES / Q2, Q2START, Q2S, Q20, Q21, Q2MUR, Q2MUF, 
cpm     1                   Q2THR, Q20F, Q21F, LAMBDA
       COMMON / SCALES / Q2, Q2START, Q20, Q21, Q2MUR, 
     1                   Q2MUF, Q2THR, LAMBDA, Q2S, Q20F, Q21F
CPM       COMMON / SCHEMEP / N       
       COMMON / SCHEME / N       

       COMMON / COUPL  / ALPS, ALP0, ALP1, ALPC, ALPB, ALPT, ALPQ, 
     1                   ALPQR, LMQ, LMQR 

cpm       COMMON / COUPLP / ALPS, ALP0, ALPC, ALPB, ALPT, ALPQ

c$$$       DATA LAMBDA / 0.248, 0.200, 0.131, 0.053 / 
c$$$       DATA Q2S /0.3/ 
c$$$       DATA Q20 / 4./                                              
c$$$       DATA Q2THR / 2.25, 20.25, 1.D8 / 

CPM       DATA Q2S, Q20 / 0.3, 4./                                              
...
c$$$       ALPS = ALPHASP (Q2S)                                                     
c$$$       ALP0 = ALPHASP (Q20)                                                     
c$$$       ALPC = ALPHASP (Q2THR(1))                                                
c$$$       ALPB = ALPHASP (Q2THR(2))                                                
c$$$       ALPT = ALPHASP (Q2THR(3))
c$$$       qq2 = q2
c$$$       ALPQ = ALPHASP (Q2)
cpm       write(*,*)"XN in PHFNLO = ", XN
CCCCCCCCCCCC
cpm       write(*,*)"ALPHAS calls in fphchsmom.f"
cpm       ALPS = ALPHAS (Q2S)     
c$$$       write(*,*)"q2s = ", q2s
cpm       write(*,*)"alps(q2s) = ", alphas(q2s)
cpm       write(*,*)"alps(q2s) = ", alps
cpm       ALP0 = ALPHAS (Q20)      
cpm       ALP0 = ALPHAS (Q20F)      
       write(*,*)"alp0 = ", alp0                                               
c$$$       write(*,*)"q2thr1 = ", Q2THR(1)
cpm       ALPC = ALPHAS (Q2THR(1)) 
       write(*,*)"alpC = ", alpc      
c$$$       write(*,*)"q2thr2 = ", Q2THR(2)                                         
cpm       ALPB = ALPHAS (Q2THR(2)) 
       write(*,*)"alpB = ", alpb                                               
c$$$       write(*,*)"q2thr3 = ", Q2THR(3)                                         
cpm       ALPT = ALPHAS (Q2THR(3))
       write(*,*)"alpT = ", alpt
cpm       qq2 = q2
c       q2 = qq2
c$$$       write(*,*)"q2 = ", q2
...
c$$$C
c$$$C                                                                               
c$$$C...CALCULATION OF ALPHA STRONG (Q**2) DIVIDED BY 4 * PI :                      
c$$$       FUNCTION ALPHASP (Q2)                                                     
c$$$       IMPLICIT DOUBLE PRECISION (A - Z)                                                    
c$$$       INTEGER NF, K                                                            
c$$$       DIMENSION LAMBDA (3:6), Q2THR (3)                                        
c$$$       COMMON / SCALESP / Q2S, Q20, Q2THR, LAMBDA                                
c$$$       NF = 3                                                                   
c$$$       DO 10 K = 1, 3                                                           
...
c$$$       B0S = B0 * B0                                                            
c$$$       B1  = 102.- 38./3.* NF                                                   
c$$$       LAM2 = LAMBDA (NF) * LAMBDA (NF)                                         
c$$$       LQ2  = DLOG (Q2 / LAM2)                                                  
c$$$       ALPHASP = 1./ (B0 * LQ2) * (1.- B1 / B0S * DLOG (LQ2) / LQ2)              
c$$$       RETURN                                                                   
c$$$       END                                                                      
...
C...AND STRUCTURE FUNCTIONS :                                                   
       ...
       DOUBLE PRECISION Q2, Q2START, Q20, Q21, Q2MUR, 
     1                  Q2MUF, Q2THR, LAMBDA, Q2S, Q20F, Q21F
       INTEGER N, F                                           
       DOUBLE PRECISION ESQ, EFO, EFS, ALP, ALPS, ALP0, ALPC, ALPB, 
     1                  ALPT, ALPQ, XL, S, XL1, ES, PI, ALP1, 
     1      ALPQR, LMQ, LMQR
CPM       COMMON / SCHEMEP / N 
       COMMON / SCHEME / N 
       COMMON / COUPL  / ALPS, ALP0, ALP1, ALPC, ALPB, ALPT, ALPQ, 
...
cpm       write(*,*)"(renop) coupl  : ",  ALPS, ALP0, ALP1, ALPC, ALPB, 
cpm     1      ALPT, ALPQ, ALPQR, LMQ, LMQR 

cpm       write(*,*)"after call to ANOMP..."
       AC  = 1.- AL  
       S   = DLOG (XL)                                                          
       XL1 = 1.- XL                                                             
       ES  = EXP (-S)                                                           
...
C...FLAVOUR DECOMPOSITION :                                                     
  70   CONTINUE                                                                 
       TNL = (SIL - NS35L) / 12.                                                
       BNL = (5.* SIL + NS35L - 6.* NS24L) / 60.                                
       CNL = (10.* SIL + 2.* NS35L + 3.* NS24L - 15.* NS15L ) / 120.            
     ...
     1        + 30.* NS3L) / 120.                                               
       TNH = (SIH - NS35H) / 12.                                                
       BNH = (5.* SIH + NS35H - 6.* NS24H) / 60.                                
       CNH = (10.* SIH + 2.* NS35H + 3.* NS24H - 15.* NS15H ) / 120.            
...
C                                                                               
      ...
      XNS = (XN - 0.3 - 1.0)                                                 
      X1S = 3.0                                                              
      XI  = (0.188 * CBETAF (XNS, X1S) * 0.5)
      XNG = XNS                                              
      X1G = DCMPLX(2.5D0, 0.D0)                                  
      GL  = 0.225 * CBETAF (XNG, X1G) * 0.5
      RETURN                                                                   
      END                                                       
C
C
...
CPM       COMMON / SCHEMEP / N
      ...
      DATA EQ / -1., 2., -1., 2., -1., 2./
      PI  = DACOS(-1.D0)
cpm       write(*,*)"message from ANOMP, F = ", F
C...PARAMETERS RELATED TO THE NUMBER OF ACTIVE FLAVOURS F:
C....BETA-FUNCTION:
c$$$      write(*,*)"%%%%%%%%%%%%% ANOMP: MAJOR DUMP %%%%%%%%%%%%%%%"
c$$$      write(*,*)ANS, AM, AP, AL, BE, AB, RPLUS, RQQ, RQG,
c$$$     1     RGQ, RGG, KNS, KSI, RKNS, RKSI, RKGL, C2Q, C1Q, C2G,
c$$$     2     C1G, C2P, C1P, ESQ, EFO, EFS,
c$$$     3     QQI, QGF, GQI, GGI, GGF, NS1PI, NS1F, QQ1F,
c$$$     4     QG1F, GQ1I, GQ1F, GG1I, GG1F, QPF, QP1F, GP1F, C2QI,
c$$$     5     C1QI, C2GF, C1GF, C2PI, C1PI, XN, F
c$$$      write(*,*)"%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
      B0  = 11. - 2./3. * F
      B02 = 2.* B0
      B1  = 102.- 38./3.* F
...
C...ANALYTIC CONTINUATIONS OF N-SUMS AS GIVEN IN GLUECK ET AL. (1990) :
       ...
       S2  = ZETA2 - PSIFN1F (N1)
       S21 = S2 + 1./N1S
       G1  = 0.5 * (PSIFN1F (N1/2.) - PSIFN1F (N/2.))
       G11 = 0.5 * (PSIFN1F (N2/2.) - PSIFN1F (N1/2.))
       SPMOM = 1.01/N1 - 0.846/N2 + 1.155/(N+3.) - 1.074/(N+4.) +
     1         0.55/(N+5.)
       SLC   = -5./8.* ZETA3
       SLV   = - ZETA2/2.* (PSIFNF (N1/2.) - PSIFNF (N/2.))
     1         + S1/NS + SPMOM
       SSCHLP = SLC + SLV
       SSTR2P = ZETA2 - PSIFN1F (N2/2.)
       SSTR3P = 0.5 * PSIFN2F (N2/2.) + ZETA3
C
...
C...PSI - FUNCTION FOR COMPLEX ARGUMENT
       DOUBLE COMPLEX FUNCTION PSIFNF (Z)
  ...
  1    CONTINUE
       ...
       RZ = 1./ ZZ
       DZ = RZ * RZ
       PSIFNF = SUB + LOG(ZZ) - RZ/2.- DZ/2520. * ( 210.+ DZ * (-21.+
     1         10.*DZ ))
       RETURN
       END
C
C...FIRST DERIVATIVE OF THE PSI - FUNCTION FOR COMPLEX ARGUMENT :
       DOUBLE COMPLEX FUNCTION PSIFN1F (Z)
       DOUBLE COMPLEX Z, ZZ, RZ, DZ, SUB
       SUB = DCMPLX (0.D0,0.D0)
       ZZ = Z
  1    CONTINUE
       ...
       RZ = 1./ ZZ
       DZ = RZ * RZ
       PSIFN1F = SUB + RZ + DZ/2. * ( 1 + RZ/630. * ( 210.- DZ * ( 42.-
     1         DZ * ( 30.- 42.*DZ ))))
       RETURN
       END
C
C...SECOND DERIVATIVE OF THE PSI - FUNCTION FOR COMPLEX ARGUMENT :
       DOUBLE COMPLEX FUNCTION PSIFN2F (Z)
       DOUBLE COMPLEX Z, ZZ, RZ, DZ, SUB
       SUB = DCMPLX (0.D0,0.D0)
       ZZ = Z
  1    CONTINUE
       ...
       RZ = 1./ ZZ
       DZ = RZ * RZ
       PSIFN2F = SUB - DZ/60. * ( 60.+ RZ * ( 60.+ RZ * ( 30.- DZ *
     1         ( 10.- DZ * ( 10.- DZ * ( 18.- 50.* DZ ))))))
       RETURN
       END
C
C...BETA FUNCTION FOR COMPLEX ARGUMENT :
       DOUBLE COMPLEX FUNCTION CBETAF (Z1, Z2)
       IMPLICIT DOUBLE COMPLEX (A - Z)
