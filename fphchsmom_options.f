...
C                                                                               
       SUBROUTINE PHFNLO(XN, Q2, Q2UP, UN, DN, SN, CN, GLN)
       IMPLICIT DOUBLE PRECISION (A - Z) 
       DOUBLE COMPLEX UN, DN, SN, CN, GLN, XN, Q2UP
       DIMENSION LAMBDA(3:6), Q2THR(3)                          
       INTEGER N
       DOUBLE PRECISION ALPS, ALP0, ALPC, ALPB,ALPT, ALPQ
       COMMON / SCALESP / Q2S, Q20, Q2THR, LAMBDA 
       COMMON / SCHEMEP / N 
       COMMON / COUPLP / ALPS, ALP0, ALPC, ALPB, ALPT, ALPQ
       
      COMMON /FLAVORS/ FLAV
c       COMMON / COUPLP / ALPS, ALP0, ALPC, ALPB, ALPT, ALPQ 
...
C...COUPLING CONSTANTS AT STATIC POINT, INPUT SCALE AND THRESHOLDS : 
       ...
       ALPQ = ALPHAS_MSTW(Q2**0.5)/(4*PI)
	
C	
       CALL RENOP (UN, DN, SN, CN, GLN, XN, Q2,Q2UP)
       RETURN
       END
C
C                                                                               
...
C...AND STRUCTURE FUNCTIONS :
       SUBROUTINE RENOP (UN, DN, SN, CN, GLN, XN, Q2,Q2UP)
       IMPLICIT DOUBLE COMPLEX (A - Z)
       INTEGER N, F,EVOLMODE
	INTEGER ICONF, FLNR, FLNR2, IEV, IEVFS
       DOUBLE PRECISION ESQ, EFO, EFS, ALP, ALPS, ALP0, ALPC, ALPB,
     1                 EVOLSW, ALPT, ALPQ, XL, S, XL1, ES, PI,Q2
...
C...FLAVOUR DECOMPOSITION :
  70   CONTINUE
       ...
       F = 5
       ALPSCOM =  ALPHASC_MSTW(Q2UP**0.5)
	IF (isnan(REAL(ALPSCOM))) THEN
	ALPSCOM = DCMPLX(1.0D0,0.0D0)
	ENDIF  
       XLCOMPLEX = ALPSCOM/DCMPLX(ALP,0)
	IF (IEVFS==3) THEN
	ALP0_LO = ALPHASC_LO(Q2**0.5)
        ALP_LO  = ALPHASC_LO(Q2UP**0.5)
       IF( isnan(REAL(ALP_LO))) THEN
       ALP_LO = DCMPLX(1.0,0.0)
        ENDIF
	XLLO = ALP_LO/ALP0_LO
	CALL FRCALCPLNNON (QQI, QGF, GQI, GGI, GGF,NS1PI, NS1F,
...
C...INHOMOGENEOUS SOLUTION :
       ...
       EM  = EXP (-AM*SCOM)
       EP  = EXP (-AP*SCOM)
       EMPL = EXP (SCOM * (-AM+AP)) - XLCOMPLEX
       EPML = EXP (SCOM * (+AM-AP)) - XLCOMPLEX
       ENS1 = ENS * ESCOM
       EM1  = EM * ESCOM
       EP1  = EP * ESCOM
       EENS = 1.- ENS
	...
	INSL = EEN1 / ANS1 * KNS / ALP0_LO
       IN3L = INSL / (3.* F * EFS)
       IN8L = IN3L
       ISIL = (EEM1 * AL / AM1  +  EEP1 * AC / AP1) * KSI / ALP0_LO
       IGLL = DCMPLX(0.D0,0.D0)
       INSH =  EEN1 / ANS1 * RPLUS * KNS
     1       + EENS / ANS * (-RPLUS * KNS + RKNS)
       IN3H = INSH / (3.* F * EFS)
       IN8H = IN3H
...
C...HOMOGENEOUS SOLUTION :
       ...
       RPPGQ = AC * DPGQ - AB * DPGG
       RPPGG = -BE * DPGQ + AL * DPGG
       HN3L = NS3L * ENS * EXP((ALP_LO-ALP0_LO) * RPLUS)
       HN8L = NS8L * ENS * EXP((ALP_LO-ALP0_LO)* RPLUS)
       HSIL = EM * EXP((ALP_LO-ALP0_LO) * RQQ) * SIL 
       HGLL = EP * EXP((ALP_LO-ALP0_LO) * RGG) * GLL
       HN3H = NS3H * ENS
       HN8H = NS8H * ENS
       HSIH = (EM * AL + EP * AC) * SIH
       HGLH  =(EM * AC + EP * AL) * GLH
	...
	ELSE
     ...
     5            C1QI, C2GF, C1GF, C2PI, C1PI, XN, F)
       ...
       INSL = EEN1 / ANS1 * KNS / ALPSCOM
       IN3L = INSL / (3.* F * EFS)
       IN8L = IN3L
       ISIL = (EEM1 * AL / AM1  +  EEP1 * AC / AP1) * KSI / ALPSCOM
       IGLL = (EEM1 / AM1  -  EEP1 / AP1) * AB * KSI / ALPSCOM
       INSH =  EEN1 / ANS1 * RPLUS * KNS
     1       + EENS / ANS * (-RPLUS * KNS + RKNS)
       IN3H = INSH / (3.* F * EFS)
       IN8H = IN3H
...
C...HOMOGENEOUS SOLUTION :
       ...
       HSIL = (EM * AL + EP * AC) * SIL  +  (EM - EP) * BE * GLL
       HGLL = (EM - EP) * AB * SIL  +  (EM * AC + EP * AL) * GLL
       HN3H = NS3L * ENS * XL1COM * RPLUS *ALPSCOM+ NS3H * ENS
       HN8H = NS8L * ENS * XL1COM * RPLUS * ALPSCOM + NS8H * ENS
       HSIH = (EM * ((RMMQQ * XL1COM + RMPQQ * EPML) * SIL
     1              + (RMMQG * XL1COM + RMPQG * EPML) * GLL)
     2        + EP * ((RPPQQ * XL1COM + RPMQQ * EMPL) * SIL
     3              + (RPPQG * XL1COM + RPMQG * EMPL) * GLL)) * ALPSCOM
     4        + (EM * AL + EP * AC) * SIH  + (EM - EP) * BE * GLH
       HGLH  = (EM * ((RMMGQ * XL1COM + RMPGQ * EPML) * SIL
     1              + (RMMGG * XL1COM + RMPGG * EPML) * GLL)
     2        + EP * ((RPPGQ * XL1COM + RPMGQ * EMPL) * SIL
     3              + (RPPGG * XL1COM + RPMGG * EMPL) * GLL)) * ALPSCOM
     4        + (EM - EP) * AB * SIH + (EM * AC + EP * AL) * GLH
...
c mbeekveld check
     ...
     6        - 20./N2
c mbeekveld difference in G11 and my G11!
...
c mbeekveld again difference in G1 function
...
C...TIMELIKE WILSON COEFFICIENTS :
     ...
     1         + 4.* (1.- 2.* N) / (NMS * NS) - 3./ N1S) * 4.
       ...
       SUBROUTINE FRCALCPLNNON(QQI, QGF, GQI, GGI, GGF,NS1PI, NS1F,
     1               QQ1F, QG1F, GQ1I, GQ1F, GG1I, GG1F, QPF, QP1F,
     2               GP1F, C2QI, C1QI, C2GF, C1GF, C2PI, C1PI, N)
       IMPLICIT DOUBLE COMPLEX (A - Z)
       DOUBLE PRECISION ZETA2, ZETA3, PI
	PI=DACOS(-1.D0)
       NS = N * N
       N1 = N + 1.
       N2 = N + 2.
...
C...LEADING ORDER :
       S1  = PSIFN (N1) + 0.577216
       LNBAR = LOG(N) + 0.577216
c mbeekveld check
       QQI = 8./3.*(-3.d0+ 4.d0*LNBAR+2.d0/N)
...
C.......MINUS COMBINATIONS ARE NOT NEEDED HERE (CF. PHOTON STRUCTURE)
c mbeekveld check
       NS1PA = -21.2203d0+(32.d0*LNBAR-18.4709d0-6.51899d0)/N
c mbeekveld check
       NS1B  = 1./18.*(-48.*PI*PI*LNBAR+1072.D0*LNBAR-52*PI*PI-129.D0)
     1     + (16.d0*LNBAR-4.D0/3.D0*PI*PI+424.D0/9.D0)/N
c mbeekveld check
       NS1C  = -176.D0/(9.D0*N)-4./9.*(40.D0*LNBAR-4.D0*PI*PI-3.D0)
c mbeekveld check
       DNS1I = (8.D0*LNBAR-6.D0)/N
c mbeekveld check
       NS1PI = -2./9.* NS1PA + 4.* NS1B - 128./9.* DNS1I
c mbeekveld check
       NS1F  = 2./3. * NS1C
...
C...PETRONZIO (1980) BY GLUECK AND REYA (1992) :
       ...
       N2T = N2S * N2
c mbeekveld check
       QQ1FS = DCMPLX(0.D0,0.D0)

c mbeekveld check
       DQQ1F = DCMPLX(0.D0,0.D0)
c mbeekveld check
       QQ1F  = QQ1FS - 16./3.* DQQ1F
c mbeekveld check
...
C...QG1 OF FP HAS TO BE MULTIPLIED BY 2 * F
c mbeekveld check
       GG1AS = -80.D0/(9.D0*N)-32.D0/9.D0*(5.D0*LNBAR-3.D0)
c mbeekveld check
       DGG1A = -1.D0/N
c mbeekveld check
       GG1A  = GG1AS + 64./3.* DGG1A
c mbeekveld check
       GG1BS = DCMPLX(8.D0,0.D0)
c mbeekveld check
       DGG1B = DCMPLX(0.D0,0.D0)
c mbeekveld check
       GG1B  = GG1BS - 8.* DGG1B
c mbeekveld check
       GG1CS = (34.594262519841024D0+32.D0*LOG(N))/N + 8.D0/3.D0*
     1     (12.463728932253974D0*LOG(N)-11.624253271961771D0)
c mbeekveld check
       DGG1C = (LNBAR-11.D0/12.D0)/N
c mbeekveld check
       GG1C  = GG1CS - 64.* DGG1C
...
c mbeekveld check
       QPF  = -3./4.*(-32.d0/(3.d0*N))
c mbeekveld check
       QP1F = 64./3.* (LNBAR*LNBAR-2.D0*LNBAR-7.D0*PI*PI/6.D0+4.D0)/N
       GP1F = DCMPLX(0.D0,0.D0)
C...TIMELIKE WILSON COEFFICIENTS :
       C2QI = 4./3.* (2.* S1 * S1 - 2.* S2 + 3.* S1 - 9.
     1       - 2.* S1 / (N * N1) + 3./ N + 4./ N1 + 2./ NS)
       DC2Q = 8./3.* (6.* S2 + 3./N1S - 3./NS - 7./2./ N - 7./2./ N1)
